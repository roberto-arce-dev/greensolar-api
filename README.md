...
Clientes e instaladores de paneles solares
...
- **InstaladorProfile** (rol: INSTALADOR)
...
```env
# MongoDB
MONGODB_URI=mongodb://localhost:27017/greensolar_db
...
```env
MONGODB_URI=mongodb+srv://usuario:password@cluster.mongodb.net/greensolar_db
...
**Rol asociado:** `INSTALADOR`

**Endpoints disponibles:**

#### Obtener mi perfil
```bash
GET /api/instalador-profile/me
Authorization: Bearer {token}
```
...
#### Listar todos los perfiles (Admin)
```bash
GET /api/instalador-profile
Authorization: Bearer {token_admin}
```

#### Obtener perfil por userId (Admin)
```bash
GET /api/instalador-profile/{userId}
Authorization: Bearer {token_admin}
```
...
## 📸 Crear ProyectoSolars con Imágenes

### ⚠️ IMPORTANTE: La imagen es OPCIONAL

La imagen **NO es requerida** al crear un proyectosolar. Puedes:
- ✅ Crear el proyectosolar SIN imagen
- ✅ Agregar la imagen DESPUÉS usando el endpoint de upload

### 🎯 Flujo Recomendado (Paso a Paso)

#### Paso 1: Crear el ProyectoSolar sin imagen

```bash
POST /api/proyectosolar
Authorization: Bearer {{token}}
Content-Type: application/json

{
  "nombre": "ProyectoSolar de ejemplo",
  "descripcion": "Descripción del proyectosolar",
  "disponible": true
}
```
...
**💡 Nota:** Guarda el `_id` del proyectosolar creado, lo necesitarás para subir la imagen.

---

#### Paso 2: Subir imagen al ProyectoSolar

```bash
POST /api/proyectosolar/507f1f77bcf86cd799439011/upload-image
Authorization: Bearer {{token}}
...
8. **Copiar** el `_id` del proyectosolar creado

---

#### Paso 2: Subir Imagen

1. **Ir a:** `Upload por Entidad` → `Upload ProyectoSolar Image`
2. **Reemplazar** `{{id}}` en la URL con el ID copiado:
   ```
   http://localhost:3007/api/proyectosolar/507f1f77bcf86cd799439011/upload-image
   ```
...
**✅ ¡Listo!** Tu proyectosolar ahora tiene imagen.
...
**Imagen original:**
```
http://localhost:3007/uploads/1700000000000-imagen.jpg
```

**Thumbnail (miniatura):**
```
http://localhost:3007/uploads/thumbnails/thumb-1700000000000-imagen.jpg
```
...
TOKEN=$(curl -X POST http://localhost:3007/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email":"productor@example.com","password":"password123"}' \
  | jq -r '.access_token')

# 2. Crear ProyectoSolar
PRODUCTO_ID=$(curl -X POST http://localhost:3007/api/proyectosolar \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{   "nombre": "ProyectoSolar de ejemplo",   "descripcion": "Descripción del proyectosolar",   "disponible": true }' \
  | jq -r '.data._id')

echo "ProyectoSolar creado con ID: $PRODUCTO_ID"

# 3. Subir imagen
curl -X POST http://localhost:3007/api/proyectosolar/$PRODUCTO_ID/upload-image \
  -H "Authorization: Bearer $TOKEN" \
...
- La URL debe empezar con `http://localhost:3007/uploads/`
...
```bash
curl -X GET http://localhost:3007/api/cliente-profile/me \
...
```bash
curl -X PUT http://localhost:3007/api/cliente-profile/me \
  -H "Authorization: Bearer {access_token}" \
  -H "Content-Type: application/json" \
  -d '{
...
**Archivo:** `greensolar-api.postman_collection.json`
...
│   │
│   ├── instalador-profile/  # Profile INSTALADOR
│   │   ├── dto/
│   │   ├── schemas/
│   │   ├── instalador-profile.controller.ts
│   │   ├── instalador-profile.service.ts
│   │   └── instalador-profile.module.ts
│   │
...
├── greensolar-api.postman_collection.json
└── package.json
```
